...
export default function MyRecipesPage() {
  ...
  const loadRecipes = useCallback(async (pageNum = page, append = false) => {
    if (pageNum > MAX_USER_RECIPES_PAGES) {
      setHasMore(false);
      return;
    }
    ...
    try {
      setLoading(true);
      const response = await fetchCurrentUserRecipes(pageNum, RECIPES_PER_PAGE);
      ...
      setError(
        err.response?.status === 404 ? 'You haven\'t created any recipes yet' :
        err.response?.status === 500 ? 'Server error. Please try again later.' :
        err.response?.status === 403 ? 'Please log in to view your recipes' :
        'Failed to load your recipes. Please try again.'
      );
  ...
  const debouncedLoadMore = useMemo(
    () => debounce(() => {
      if (hasMore && !loading && page <= MAX_USER_RECIPES_PAGES) {
        setPage(prev => prev + 1);
      }
    }, 150),
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      {hasMore && page <= MAX_USER_RECIPES_PAGES && (
        <div id="scroll-target" className="h-4 w-full" />
      )}

      {(!hasMore || page > MAX_USER_RECIPES_PAGES) && recipes.length > 0 && (
